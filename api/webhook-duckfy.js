...
export default async function handler(req, res) {
  ...
  try {
    ...
    const amountCents = Number(tx.amount) || 0; // Duckfy webhook manda em CENTAVOS
    const amountReais = amountCents / 100;       // Converte pra reais
    ...
    // TRANSACTION_CREATED: ignora — o waiting_payment já foi enviado pelo create-pix.js
    // (se a gente disparar aqui também, UTMify vê 2 pedidos)
    if (/TRANSACTION_CREATED/i.test(event)) {
      console.log('[webhook] TRANSACTION_CREATED ignorado (waiting já enviado no create-pix)');
      return res.status(200).json({ received: true, note: 'created_ignored' });
    }

    // Só dispara paid se for ESTRITAMENTE TRANSACTION_PAID
    // (sem regex defensiva que casa com qualquer coisa — evita duplicação)
    const isPaid = /TRANSACTION_PAID/i.test(event);
